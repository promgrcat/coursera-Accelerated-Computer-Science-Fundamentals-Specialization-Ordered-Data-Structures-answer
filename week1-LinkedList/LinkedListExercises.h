...
void LinkedList<T>::insertOrdered(const T& newData) {
  ...
  if (!head_){
    ...
    Node *cur = head_;
    //std::cout << "this point ok";
    while (cur){
      // if current data less than new data travel to next point.
      if (cur->data < newData){
        if (cur->next) {
        cur = cur->next;
          
        }
        // if travel the whole thing and in the tail
        else{
          cur->next = newNode;
          newNode->prev = cur;
          tail_ = newNode;
      ...
      // if current data >= new data then do tne insertion
      else{
        // if it should be insert into the front
        if (!cur->prev){
          cur->prev = newNode;
          newNode->next = cur;
          head_ = newNode;
...
LinkedList<T> LinkedList<T>::merge(const LinkedList<T>& other) const {
  ...
  // since in the splitHalves, left.size_ >= right.size_
  // if right.size_ == 0, we can just copy left to merged



  int size = left.size_ + right.size_;
    
  int i = 0;
  while (i < size){
      //If left.size_ == 0 during loop, then just copy right to merged
    if (left.size_ == 0) {
      int right_size = right.size_;
      for (int i = 0; i <right_size;i++){
        T dataToCopy = right.front();
        merged.pushBack(dataToCopy);
        right.popFront();
      }
      ...
      //Same as above
    if (right.size_ == 0) {
      int left_size = left.size_;
      for (int i = 0; i <left_size;i++){
        T dataToCopy = left.front();
        merged.pushBack(dataToCopy);
        left.popFront();
      } 
      break;
